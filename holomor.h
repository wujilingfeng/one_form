...
public:
	...
	float interal_on_face(Eigen::MatrixXf &w1, Eigen::MatrixXf &w2)
	{
		...
		for (PMyMesh::FaceIter fter = mesh.faces_begin(); fter != mesh.faces_end(); fter++)
		{
			...
			sum += (n.transpose()*(temp1.cross(temp2))).coeff(0)*area;
	...
	void one_omigastar(int ii)
	{
		...
		for (int i=0;i<g_2;i++)
		{
			b.coeffRef(i) = interal_on_face(w1[i],w2);
			printf_s("b:%f\r\n",b(i));
		}
		x = ww.inverse()*b;
		w3.setZero();
		...
		for (PMyMesh::HalfedgeIter hter=mesh.halfedges_begin();hter!=mesh.halfedges_end();hter++)
		{
			...
			for (int i=0;i<g_2;i++)
			{
				sum += mesh.data(*hter).omiga[i] * x(i);

			}
			mesh.data(*hter).omiga_star[ii] = sum;
		}
		//for (int i = 0; i<g_2; i++)
		//{
		//	//sum += x(i)*ww(0, i);
		//	printf_s("x:%f\r\n", x(i));
		//}
		//Eigen::SimplicialLDLT<Eigen::MatrixXf>ldlt;
		//ldlt.compute(ww);
		//x=ldlt.solve(b);
	//	printf_s("size:%d\r\n",ww.cols());
	...
	//set_cotangentbasis();
		...
		//Eigen::MatrixXf w1(mesh.n_faces(),3), w2(mesh.n_faces(), 3);
		std::vector<Eigen::MatrixXf> w1;
		for (int i=0;i<g_2;i++)
		{
			Eigen::MatrixXf tempm;
		...
		for (int i=0;i<g_2;i++)
		{
			for (int j=0;j<g_2;j++)
			{
				//ww.coeffRef(i, j) = interal_on_face(w1[i], w1[j]);
				if (j>i)
				{
					ww.coeffRef(i, j) = interal_on_face(w1[i],w1[j]);
