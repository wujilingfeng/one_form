...
struct MyTraits :OpenMesh::DefaultTraits
{
	VertexTraits{
		int layer = -1;
	int key_ = 0; 
	float distance = -1,f_ = 0,area = 0,u=0,v=0;
	
	};
	EdgeTraits{
		float length = -1;
	bool sharp_ = false,sharp1_=false;
	float weight = -1;
	
	};
	HalfedgeTraits{
		float omiga[N] = { 0 },omiga_star[N] = {0};
	};
	FaceTraits
	{
		Eigen::MatrixXf w,w_star;
	};
...
public:
	...
	void find_nextlayer(VertexHandle v)
	{
		...
		float distance = mesh.data(v).distance;
		for (PMyMesh::VertexIHalfedgeIter vheter = mesh.vih_begin(v); vheter.is_valid(); vheter++)
		{
			...
			if (mesh.data(v1).layer == -1)
			{
				...
				mesh.data(v1).distance = mesh.data(mesh.edge_handle(*vheter)).length + distance;
				//mesh.data(v1).u=mesh.data(v).u+mesh.data(halfedgeh(v,v1,mesh))
				mylst1.push_back(v1);

			}
			else if (mesh.data(v1).layer == (layer + 1))
			{
				float temp = mesh.data(mesh.edge_handle(*vheter)).length + distance;
				if (temp < mesh.data(v1).distance)
				{
					mesh.data(v1).distance = temp;
				}
			}
			else if (mesh.data(v1).layer > layer + 1)
			{
	...
	void find_shortestway(VertexHandle v)
	{
		...
		for (temp = mesh.data(vvter).distance; vvter.is_valid(); vvter++)
		{
			if (mesh.data(vvter).distance < temp)
			{
				temp = mesh.data(vvter).distance;
				vtemp = (*vvter);
			}
	...
	void setkey()
	{
		...
		for (PMyMesh::VertexIter vter = mesh.vertices_begin(); vter != mesh.vertices_end(); vter++)
		{
			if (mesh.data(vter).key_ >= 4)
			{
				vh[nfa] = (*vter);
				//cout << "原马鞍点为：" << (*vter).idx() << " key  " << mesh.data(vter).key_ << endl;
				printf_s("原马鞍点为： %d,key为 %d\r\n", (*vter).idx(), mesh.data(vter).key_);
				//findsaddle(false, vh);
				//break;
				nfa++;
			}
	...
	void findsaddle(PMyMesh::VertexHandle v)
	{
		printf_s("下一马鞍点为： %d,key为 %d\r\n", v.idx(), mesh.data(v).key_);
	...
	void drawedge()
	{
		...
		for (PMyMesh::EdgeIter eter = mesh.edges_begin(); eter != mesh.edges_end(); eter++)
		{
			if (mesh.data(*eter).sharp_ || mesh.data(*eter).sharp1_)
			{
	...
	void set_frame()
	{
		...
		viewer.ngui->addVariable<bool>("boolvariable", [&](bool var) {boolvariable = var;  },
			[&]() {return boolvariable; });
		viewer.ngui->addButton("drawedge", [&]() {viewer.data.clear(); init_sharp(); findsaddle(vh[kk]); drawedge(); draw_scene(); });

		viewer.screen->performLayout();
		return false;
		};
	}
	void find_next(VertexHandle v,int ii)
	{
		VertexHandle v1;
		int layer = mesh.data(v).layer;
		...
		for (PMyMesh::VertexIHalfedgeIter vheter = mesh.vih_begin(v); vheter.is_valid(); vheter++)
		{
			...
			if (mesh.data(v1).layer == -1)
			{
				...
				mesh.data(v1).u = mesh.data(v).u + mesh.data(halfedgeh(v, v1, mesh)).omiga[ii];
				mesh.data(v1).v = mesh.data(v).v + mesh.data(halfedgeh(v, v1, mesh)).omiga_star[ii];
				mylst1.push_back(v1);
	...
	void integral(VertexHandle v,int ii)
	{
		mesh.data(v).layer = 0;
		//mesh.data(v).distance = 0;
		mylst0.push_back(v);
	...
	void pre_draw1()
	{
		if (viewer.core.is_animating)
		{
			if (!mylst0.empty())
			{
				...
				for (PMyMesh::VertexIter vter = mesh.vertices_begin(); vter != mesh.vertices_end(); vter++)
				{
					if (mesh.data(*vter).u!=0)
					{
						z.coeffRef((*vter).idx()) = 2;
					}
					//cout << (*vter).idx() << "   " << mesh.data(*vter).key_ << "   " << mesh.data(vter).f_ << endl;
				}
				Eigen::MatrixXd c;
